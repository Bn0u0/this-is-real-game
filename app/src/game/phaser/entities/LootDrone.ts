...
export class LootDrone extends Phaser.GameObjects.Container {
    ...
    startSpawnAnim() {
        ...
        this._scene.tweens.add({
            ...
            onComplete: () => {
                ...
                EventBus.emit('SHOW_TOAST', '?©è??¡äººæ©Ÿå·²?µé? (Supply Drone Arrived)');
            }
        });
    }
    ...
    tick(time: number, delta: number, player: Player) {
        ...
        if (dist < this.radius) {
            ...
            if (this.droneState === DroneState.CHARGING) {
                ...
                this.chargeTimer = 0;
                this.drawProgress(0);
                EventBus.emit('SHOW_TOAST', 'ä¸Šå‚³ä¸­æ–· (Upload Aborted)');
            }
        }
    }
    ...
    upload() {
        ...
        EventBus.emit('SHOW_TOAST', `ä¸Šå‚³?å?: ${count} ?©å? (Upload Complete)`);
